...
from configs import dualSRNetArgs
...
if args.phase == 'train':
    wandb.init(
        project="dualSRNet",     # change this to your actual wandb project name
        name=args.modelName,     # run name = model name
        config=vars(args),       # automatically logs all hyperparameters from args
    )
...
with strategy.scope():
    ...
    def wandbSliceImage(arr):
        # takes a central 2D slice from a volume/array for visualization in wandb
    ...
    if args.phase == 'train':
        ...
        for epoch in range(EPOCHS):
            ...
            wandb.log({
                'epoch': epoch+1,
                'lr': lr,
                'train/G_SRxy_loss': totGABL,
                'train/G_SRyz_loss': totGBAL,
                'train/adv_SRxy_loss': totADVXYSRL,
                'train/adv_SRyz_loss': totADVYZSRL,
                'train/D_SRxy_loss': totDXYSRL,
                'train/D_SRyz_loss': totDYZSRL,
            }, step=epoch+1)
            ...
            if np.mod(epoch+1, args.print_freq) == 0 or epoch == 0:
                ...
                for C, B in zip(HR_dataset_test, LR_dataset_test):
                    ...
                    if numTestBatches == 1:
                        wandb.log({
                            'val_images/Bxy_input': wandbSliceImage(B),
                            'val_images/Cxyz_target': wandbSliceImage(Co),
                            'val_images/Ctxyz_target': wandbSliceImage(C),
                            'val_images/SRxy_output': wandbSliceImage(fakeCo),
                            'val_images/SRxyz_output': wandbSliceImage(fakeC_clean),
                        }, step=epoch+1)
                ...
                wandb.log({
                    'val/PSNR_SR': valPSNRC,
                    'val/PSNR_SRC': valPSNRCC,
                }, step=epoch+1)
