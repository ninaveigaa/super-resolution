"""
scripts/Wang_2023_process_data_overlap.py
...
Pipeline:
    ...
    raw .tiff (uint16), loaded as [Z, Y, X] by tifffile
        ...
        --> splitting: 90% train / 20% validation 

Usage (from the repo root):
    python scripts/Wang_2023_process_data_overlap.py
"""
...
OUT_DIR = Path("data/processed/Wang_2023_9020")
...
TRAIN_FRACTION = 0.9
VAL_FRACTION = 0.2
OVERLAP = TRAIN_FRACTION + VAL_FRACTION - 1
...
def prepare_data():
    ...
    section(f"Splitting: {int(TRAIN_FRACTION*100)}% train / "
            f"{round(VAL_FRACTION*100)}% validation "
            f"({round(OVERLAP*100)}% overlap)")
    lr_split = preprocessing.splitting(lr_cropped, size=TRAIN_FRACTION, overlap=OVERLAP,
                                        output_dir=OUT_DIR / "_steps", base_name="LR_05_split")
    hr_split = preprocessing.splitting(hr_cropped, size=TRAIN_FRACTION, overlap=OVERLAP,
